import {get, div, button, input, renderComponent, a, assertEqual} from './helpers.js'
...
export const ServerList = ({page, pageSize, list, addServer, userName}) => 
  div({className: 'serverList'}, 
    trimList(page, pageSize, list
    .map((name) => ServerPreview({name, userName}))
    // Add a window for managing servers
    .concat(AddServerWindow(addServer)) 
...
//  '/api/v1/trends/statuses?&limit=40'
    ...
    .then((result) => {
      ...
      container.replaceChildren(...resultSorted
        .map((post) => Post({post, userName, name})))
    })
...
const Post = ({post, userName, name}) => {
  ...
  const userHref = `${serverHref}@${post.account.username}${userName.server !== name ? '@' + name : ''}`
  ...
  content.innerHTML = post.content
    ...
    https://mastodon.art/users/datGestruepp/statuses/111209048265884242
  ...
  return div({className: 'post'}, [
    ...
    content,
    a({text: `⤺${post.replies_count} ★${post.favourites_count} ⇅${post.reblogs_count}`, href:`${postHref}`, target: '_blank'})
  ])
}
